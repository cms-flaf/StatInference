...
def step_y(hists, signal_name, bkg_names, x_low, x_high, y_low, y_high):
    ...
    sum_signal[0] = integrate_uproot(hists[signal_name], x_low, x_high, new_y_low[0], new_y_high[0])[0]

    sum_signal[1] = integrate_uproot(hists[signal_name], x_low, x_high, new_y_low[1], new_y_high[1])[0]
    sum_signal[2] = integrate_uproot(hists[signal_name], x_low, x_high, new_y_low[2], new_y_high[2])[0]

    sum_background_current = 0

    for bkg_name in bkg_names:
        sum_background[0] += integrate_uproot(hists[bkg_name], x_low, x_high, new_y_low[0], new_y_high[0])[0]

        sum_background[1] += integrate_uproot(hists[bkg_name], x_low, x_high, new_y_low[1], new_y_high[1])[0]
        sum_background[2] += integrate_uproot(hists[bkg_name], x_low, x_high, new_y_low[2], new_y_high[2])[0]
...
def validate_bin(hists, bkg_names_main, bkg_names, x_low, x_high, y_low, y_high):
    ...
    tot_background = 0
    for bkg_name in bkg_names:
        tot_background += integrate_uproot(hists[bkg_name], x_low, x_high, y_low, y_high)[0]
    if tot_background <= 0:
        return False
...
def integrate_uproot(hist, start_x, end_x, start_y=0, end_y=0):
    ...
    if len(values.shape) == 1:
        ...
        int_errors = np.sqrt(np.sum((errors[start_x:end_x+1,start_y:end_y+1])**(2.0)))
...
def optimize_binning(shapes_dir, filename, sig_string, params, mass_list, outdir, config, nBinsMaxX, nBinsMaxY, background_names, important_backgrounds, categories, channels, tag):
    ...
    for cat in categories:
        for ch in channels:
            ...
            bins_dict[dict_key] = [{'y_bin': [np.float64(0.0), np.float64(2500.0)], 'x_bins': [0, 1.0]}]
    ...
    for ch, cat in priority_list:
        ...
        for low_y in range(y_upper_limit, y_lower_limit-1, -1):
            if np.sum(total_signal_y_slices[low_y:current_upper_limit+1]) >= signal_per_bin_y:
        ...
        for y_index in range(len(y_binning) - 1):
            final_binning.append(
                {"y_bin": [y_binning[y_index], y_binning[y_index+1]], "x_bins": [x_edges[0], x_edges[x_max]]}
            )
        ...
        for y_index in range(len(y_binning) - 1):
            print(f"Going to check the y range {y_binning[y_index]} to {y_binning[y_index+1]}")
            print(f"Or in indexes, {y_binning_indexes[y_index]} to {y_binning_indexes[y_index+1]}")


            # #Start the loop over nBins from 1 to nBinsMax
            signal_this_slice = np.sum(total_signal_y_slices[y_binning_indexes[y_index]:y_binning_indexes[y_index+1]])
            for nbins in range(6, nBinsMaxX):
                bin_content_goal = signal_this_slice/nbins

                print(f"Channel {ch_cat} has a total of {signal_this_slice} entries in this Y, so with {nbins} bins our per-bin goal is {bin_content_goal}")
                ...
                while not done:
                    for left_edge in range(right_edge, -1, -1):
                        current_total_signal = integrate_uproot(hists[signal_name], left_edge, right_edge, y_binning_indexes[y_index], y_binning_indexes[y_index+1]-1)[0]
                        current_total_backgrounds = 0

                        for bkg_name in bkg_names:
                            current_total_backgrounds += integrate_uproot(hists[bkg_name], left_edge, right_edge, y_binning_indexes[y_index], y_binning_indexes[y_index+1]-1)[0]



                        if current_total_signal >= bin_content_goal:
                            print(f"Achieved bin content goal {current_total_signal} >= {bin_content_goal}, check if backgrounds are valid")
                            negative_flag = False
                            for bkg_name in bkg_names_main:
                                if integrate_uproot(hists[bkg_name], left_edge, right_edge, y_binning_indexes[y_index], y_binning_indexes[y_index+1]-1)[0] <= 0:
...
if __name__ == '__main__':
        ...
        parser.add_argument('--nBinsMaxY', required=False, type=int, default=5, help="Maximum number of bins to test")
        parser.add_argument('--nBinsMaxX', required=False, type=int, default=7, help="Maximum number of bins to test")
        ...
        for mass in mass_list:
            hists = optimize_binning(shapes_dir, file_name_pattern.format(m = mass), signal_name_pattern.format(m = mass), {'MX': mass}, mass, outdir, config, nBinsMaxX, nBinsMaxY, background_names, important_backgrounds, categories, channels, tag)
            # break